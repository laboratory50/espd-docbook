 ...
 *  Apache FOP версии 1.1, рекомендуется актуальная версия из репозитория.

 *  DocBook XSL версии не менее 1.78.

 *  Для работы с русским языком крайне рекомендуется использовать FOP в
    связке с Saxon вместо штатного для многиз дистрибутивов Xalan.
...
файл Makefile, с помощью которого все это можно собрать. Это хороший тест
на правильность настройки вашей среды. Но должны быть установлены
соотвествующие шрифты (перечислены в Makefile).
